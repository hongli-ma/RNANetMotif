    ...
    for i in range(0,len(lines),2):
        ...
        with open(edges_files[ind]) as ed:
            for line in ed:
                ...
                LIST_edge_temp.append(values)
        LIST_EDGE.append(LIST_edge_temp)
df = pd.DataFrame(list(zip(LIST_NAME, LIST_SEQ,LIST_EDGE)), columns =['chr_start_end', 'sequence','base_pair_weight']) 
df.to_pickle(RBP+'_seq_edges.pkl')           
